...
TestsOf("Block Rules: ", {
  ...
  'HeadingSection captures under':
    [ () => {
      ...
      assertEquals( BuiltBlockRules['HeadingSect'].regex.exec( headingSectionKeyOut )?.groups?.RESULT, 
        'Heading 1: Title of Section\n' );
      assertEquals( BuiltBlockRules['HeadingSect'].regex.exec( headerOnlyCapture )?.groups?.RESULT, 
        'Heading 1: Title of Section\n' );
    }, 'only selecting the header text.'],

  'Text captures multilines     ':
    [ () => {
...
TestsOf("Inline Rules: Highlight - ", {
  ...
  'captures RESULT              ':
    [ () => {
      ...
      assertEquals( BuiltInlineRules['Highlight'].regex.exec( highlightDefault )?.groups?.RESULT, 
        'highlighted Text - Hello' );
      assertEquals( BuiltInlineRules['Highlight'].regex.exec( highlightWType )?.groups?.RESULT, 
        'highlighted Result! ~  ' );
    }, 'contains text.'],
