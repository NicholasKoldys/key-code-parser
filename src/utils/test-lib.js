...
function getCaseStrings(results) {
    const caseArray = [];
 ...
 * @param {Suite} suite
 */
export function TestsOf(suite, isReturned = false) {
    let failures = 0;
...
export function FastTestOf(suite) {
    try {
        ...
        if (err instanceof Error) {
            fLog('Test:', err.cause, 'FAILED', err.message);
            console.error(err.stack);
        }
    }
}
export function assignTestResults(result, testCase, expression) {
 ...
 */
export function assert(expression, testCase = "test case", result) {
 ...
 * @param {any} actual
 */
export function assertEquals(expected, actual, testCase, result) {
 ...
 * @param {any} actual
 */
export function assertStrictEquals(expected, actual, testCase, result) {
    result ? assignTestResults(result, testCase, [expected, actual]) : null;
    if (expected !== actual) {
